...
set<Instruction *>SensitiveChecksPass::ErrSelectInstSet;

/// Check if the value is an errno.
bool SensitiveChecksPass::isValueErrno(Value *V, Function *F) {
	// Invalid input.
	if (!V)
		return false;
...
void SensitiveChecksPass::dumpErrEdges(EdgeErrMap &edgeErrMap) {
...
/////////////////////////////////////////////////////////////////////
// Implementation of SensitiveChecksPass
/////////////////////////////////////////////////////////////////////

/// Find error code based on error handling functions
void SensitiveChecksPass::findErrorCodes(Function *F) {
#if 0
...
/// Incorporate flags
void SensitiveChecksPass::updateReturnFlag(int &errFlag, int &newFlag) {
...
void SensitiveChecksPass::updateHandleFlag(int &errFlag, int &newFlag) {
...
void SensitiveChecksPass::mergeFlag(int &errFlag, int &newFlag) {
...
void SensitiveChecksPass::markBBErr(BasicBlock *BB, 
...
void SensitiveChecksPass::recurMarkEdgesFromBlock(CFGEdge &CE, int flag, 
...
void SensitiveChecksPass::recurMarkEdgesToBlock(CFGEdge &CE, int flag, 
		BBErrMap &bbErrMap, EdgeErrMap &edgeErrMap) {
...
/// closest branches
void SensitiveChecksPass::recurMarkEdgesToErrHandle(BasicBlock *BB, 
...
void SensitiveChecksPass::recurMarkEdgesToErrReturn(BasicBlock *BB, 
...
void SensitiveChecksPass::markEdgesToErrReturn(BasicBlock *BB, 
		int flag, EdgeErrMap &edgeErrMap) {
...
bool SensitiveChecksPass::markAllEdgesErrFlag(Function *F, BBErrMap &bbErrMap, 
		EdgeErrMap &edgeErrMap) {
...
/// error
bool SensitiveChecksPass::mayReturnErr(Function *F) {
...
void SensitiveChecksPass::checkErrReturn(Function *F, 
		BBErrMap &bbErrMap) {
...
void SensitiveChecksPass::checkErrHandle(Function *F, 
		BBErrMap &bbErrMap) {
...
void SensitiveChecksPass::identifySecurityChecks(Function *F, 
		EdgeErrMap &edgeErrMap,
	...
	// may need to be promoted to SensitiveChecksPass.
...
void SensitiveChecksPass::checkErrValueFlow(
		Function *F,
		ReturnInst *RI, 
		std::set<Value *> &PV, 
		BBErrMap &bbErrMap) {
...
void SensitiveChecksPass::addSecurityCheck(Value *SC, Value *Br,
		set<SecurityCheck *> &SCSet) {
...
void SensitiveChecksPass::findSameVariablesFrom(Value *V, 
		std::set<Value *> &VSet) {
...
int SensitiveChecksPass::inferErrBranch(Instruction *Cond) {
...
bool SensitiveChecksPass::doModulePass(Module *M) {
...
/// Traverse the CFG and find security checks.
void SensitiveChecksPass::identifySensitiveChecks(Function *F, 
		Instruction *I,
		EdgeErrMap &edgeErrMap,
		set<SecurityCheck *> &SCSet) {
...
bool SensitiveChecksPass::IsSecurityCheck(Function *F, Instruction *I) {
	if (F->empty())
		return false;
	...
	set<SecurityCheck *> SCSet; 
	// Traverse the CFG and find security checks for each errno.
	identifySensitiveChecks(F, I, edgeErrMap, SCSet);

	if (SCSet.empty()) return false;

	
	Ctx->NumSecurityChecks += SCSet.size();

	// for (auto SC : SCSet) {
	// 	if(Ctx->SecurityCheckSets.find(F) == Ctx->SecurityCheckSets.end()){
	// 		Ctx->SecurityCheckSets[F].insert(*SC);
	// 	}
	// 	if(Ctx->CheckInstSets.find(F) == Ctx->CheckInstSets.end()){
	// 		Ctx->CheckInstSets[F].insert(SC->getSCheck());
	// 	}
	// }
